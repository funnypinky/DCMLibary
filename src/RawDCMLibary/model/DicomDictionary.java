 ...
 * assume the dictionary file is found in the working directory of mipav or in
 * the root of the jar file MIPAV is being run from (see the
 * ClassLoader.getResource() method, with the default name
 * &quot;dicom_dictionary.txt&quot;.
 *
 * <p>
 * The dictionary defines each tag on its own line, begining with the tag's ID
 * in parenthesis, for example: <code>
 * (01AC,1101)</code>, where the initial 4 digits are the group number, and the
 * second for digits are the element number. Following the group and element,
 * each attribute of the tag definition is specified by attribute type, followed
 * by an &quot;=&quot;, then the attribute value in quotations. The attributes
 * are separated by tabs, and are held in order:
 * </p>
 *
 * <ol>
 ...
 * <li>It has a static method to write out a dicom dictionary out to a
 * file.</li>
 * </ul>
 *
...
public class DicomDictionary {
     ...
     */
    private static HashMap<FileDicomKey, FileDicomTagInfo> masterHashtable;

    /**
     * Hashtable filled with DICOM tags which are a subset (not necessarily a
     * proper subset) of dicom tags in the master table. This subset is then
     * used to export dicom tags to the XML image format.
     */
    private static HashMap<FileDicomKey, FileDicomTagInfo> subsetHashtable;

    // ~ Methods
    // --------------------------------------------------------------------------------------------------------
    /**
     * Returns whether the dicom dictionary contains a tag with the given key
     * identifier.
     *
     * @param key the key for this tag
     *
     * @return whether a tag matching the given key is contained in the dicom
     * dictionary.
     * @throws java.lang.CloneNotSupportedException
     */
    public static boolean containsTag(FileDicomKey key) throws CloneNotSupportedException {
     ...
     * @throws java.lang.CloneNotSupportedException
     */
    public static HashMap<FileDicomKey, FileDicomTagInfo> getDicomTagTable() throws CloneNotSupportedException {
     ...
     * Returns a reference to the DICOM Hashtable.
     *
     * @param forceReload If true, forces the master tag table to be re-read
     * from the dicom dictionary file
     *
     * @return a reference to the dicom tag table
     * @throws java.lang.CloneNotSupportedException
     */
    public static HashMap<FileDicomKey, FileDicomTagInfo> getDicomTagTable(final boolean forceReload) throws CloneNotSupportedException {

        if ((DicomDictionary.masterHashtable == null) || (forceReload == true)) {
            DicomDictionary.parseFile(DicomDictionary.DEFAULT_DICTIONARY);
        }

        final HashMap<FileDicomKey, FileDicomTagInfo> clonedHashtable = new HashMap<>(
                (int) (DicomDictionary.masterHashtable.size() / 0.7));
        final Enumeration<FileDicomKey> e = (Enumeration<FileDicomKey>) DicomDictionary.masterHashtable.keySet();
     ...
     * @throws java.lang.CloneNotSupportedException
     */
    public static FileDicomTagInfo getInfo(FileDicomKey key) throws CloneNotSupportedException {
 ...
 if it no key is found.
     ...
     * @throws java.lang.CloneNotSupportedException
     */
    public static String getKeyFromTagName(final String searchTagName) throws CloneNotSupportedException {
        final Enumeration<FileDicomKey> enumeration = (Enumeration<FileDicomKey>) DicomDictionary.masterHashtable.keySet();
     ...
     * &quot;dicom_dictionary.txt&quot;). if the tag is not in the hashtable,
     * <code>null</code> is returned.
     * @throws java.lang.CloneNotSupportedException
     */
    public static String getKeyword(FileDicomKey key) throws CloneNotSupportedException {
     ...
     * @throws java.lang.CloneNotSupportedException
     */
    public static String getName(FileDicomKey key) throws CloneNotSupportedException {
    ...
    /**
     * Quietly checks to see if the subset dicom dictionary exists on disk. Can
     * be used to avoid a call to getSubsetDicomTagTable() that might cause a
     * warning dialog to appear.
     *
    ...
    public static boolean doesSubsetDicomTagTableExist() {
        ...
        if (dictionaryReference == null) {
            System.err.println("Failed to read DICOM dictionary file from " + DicomDictionary.SUBSET_DICTIONARY_FILENAME);
     ...
     */
    public static HashMap<FileDicomKey, FileDicomTagInfo> getSubsetDicomTagTable() {
     ...
     * Returns a reference to the subset dicom tag table.
     *
     * @param forceReload If true, forces the subset tag table to be re-read
     * from the dicom save dictionary file
     *
     * @return A reference to the subset dicom tag table
     */
    public static HashMap<FileDicomKey, FileDicomTagInfo> getSubsetDicomTagTable(final boolean forceReload) {
    ...
    /**
     * Accessor that returns the type of the tag (different from, but related to
     * the vr).
     *
     * @param key the key of the desired name.
     *
     * @return the type if the tag is not in the hashtable, <code>null</code> is
     * returned.
     * @throws java.lang.CloneNotSupportedException
     */
    public static VR getType(FileDicomKey key) throws CloneNotSupportedException {
 ...
 there can be in one tag.
     ...
     * @throws java.lang.CloneNotSupportedException
     */
    public static int getVM(FileDicomKey key) throws CloneNotSupportedException {
 ...
 representation allows the reader the read and interpret the tag properly. Because private tags are not unique, the VR is null and they may be read
 and/or displayed improperly.
     ...
     * @throws java.lang.CloneNotSupportedException
     */
    public static VR getVR(FileDicomKey key) throws CloneNotSupportedException {
    ...
    /**
     * Sorts the list of tags and returns it as an array in order of
     * FileDicomKeys.
     *
     * @param dicomTagsList The hashtable of DICOM tags (the keys are
     * FileDicomKey and the object referred to by that key is the
     * FileDicomTagInfo).
     *
     * @return a sorted array of DICOM Keys.
     */
    public static FileDicomKey[] sortTagKeys(final HashMap<FileDicomKey, FileDicomTagInfo> dicomTagsList) {
        FileDicomKey[] dicomKeys;

        dicomKeys = new FileDicomKey[dicomTagsList.size()];

        int q = 0;
        final Enumeration<FileDicomKey> e = (Enumeration<FileDicomKey>) dicomTagsList.keySet();
        ...
        final Comparator<FileDicomKey> comparator = (final FileDicomKey key1, final FileDicomKey key2) -> {
            int key1GN, key2GN;
            try {
     ...
     */
    public static void writeFile(final File dictFile, final HashMap<FileDicomKey, FileDicomTagInfo> dicomHash)
            throws IOException {
    ...
    public static void writeFile(final File dictFile, final HashMap<FileDicomKey, FileDicomTagInfo> dicomHash,
        ...
        try (FileWriter fw = new FileWriter(dictFile, false)) {
     ...
     */
    private static FileDicomKey convertToWildKey(final FileDicomKey key) throws CloneNotSupportedException {
    ...
    private static BufferedReader getFileReader(final String filename) {
        ...
        String filepath;
        String tp = new StringBuilder("misc/").append(filename).toString();

        if (filename.equals(DicomDictionary.DEFAULT_DICTIONARY_FILENAME)) {
            
            InputStream inputStream = DicomDictionary.class.getResourceAsStream(tp);
        ...
        if (!dictionaryFile.exists()) {
            try {
                ...
                Logger.getLogger(DicomDictionary.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
...
/**
 * Method called once when the user opens MIPAV. It parses the dictionary file,
 * normally called &quot;dicom_dictionary.txt&quot;. The dictionary file is
 * where all the tags are listed and stores these in the DICOMHashtable, with
...
private static void parseFile(final int dictionary_type) {
        ...
        final HashMap<FileDicomKey, FileDicomTagInfo> hashtable = new HashMap<>();
        ...
        if (dictionaryReference == null) {
            System.out.println("Failed to read DICOM dictionary file from " + filename);
            System.out.println("If saving a dicom image, no tags will be saved.");
            return;
        }
        ...
        try {
            ...
            while ((s = dictionaryReference.readLine()) != null) {
                ...
                try {
                    vm = Integer.valueOf(vmS);
                } catch (final NumberFormatException e) {

                    if (vmS.endsWith("n")) {
                        vm = Integer.MAX_VALUE;
                    } else {
                        vm = Integer.valueOf(vmS.substring(2));
                    }
                }
            ...
            System.err.println("Failed to parse DICOM dictionary file.");
            e.printStackTrace();
        } finally {
