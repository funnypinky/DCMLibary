 ...
 * It reads in a buffer of tags that can be parsed more quickly than continued
 * random accesses to the harddrive.
 *
...
public class FileDicomBase {
    ...
    /**
     * The size of the buffer that contains the tags of the DICOM image. Default
     * = 400Thousand.
     */
    ...
    //~ Instance fields ------------------------------------------------------------------------------------------------
    /**
     * One byte array used to read/write in data so that one doesn't't need to
     * be allocated with each read/write.
     */
    protected final byte[] byteBuffer = new byte[1];

    /**
     * Two byte array used to read/write in data so that one doesn't't need to
     * be allocated with each read/write.
     */
    protected final byte[] byteBuffer2 = new byte[2];

    /**
     * Four byte array used to read/write in data so that one doesn't need to be
     * allocated with each read/write.
     */
    protected final byte[] byteBuffer4 = new byte[4];

    /**
     * Eight byte array used to read/write in data so that they don't need to be
     * allocated with each read/write.
     */
    ...
    /**
     * Integer variable used to read/write in data so that they don't need to be
     * allocated with each read/write.
     */
    ...
    /**
     * The number of images inside of this file which are not the main
     * displayable image (may be icon, RT planning, etc.
     */
    ...
    public void finalize() {
        try {
            ...
            try {
                ...
                Logger.getLogger(FileDicomBase.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
    }
     ...
     *
     * @param endianess  <code>true</code> indicates big endian byte order,
     ...
     * <code>false</code> indicates little endian.
     *
     * @return The value of unsigned short read from the file returned as an
     * int.
     *
     * @exception IOException if there is an error reading the file
     */
    ...
    /**
     * Performs internal search for the image tag in a dicom file, given an
    ...
    private boolean searchForImageTag(byte[] tempTagBuffer, byte[] endianBuffer, int bufferIndex) {
        ...
        if (tempTagBuffer[bufferIndex] == endianBuffer[0]) {
            int j = 0;
            tagSearch:
            for (j = 1; j < endianBuffer.length; j++) {
    ...
    /**
     * Locates a DICOM image beginning at the given offset.When multiple images
 after the offset exist, imageNumber can be used to specify the exact
 image to retrieve.
     * @param offset
     * @param imageNumber
     * @return 
    ...
    public int locateImageTag(int offset, int imageNumber) {
        ...
        seek(bPtr);
        long raFileLength = 0;
        int bufferLength = (int) (tagBuffer.length < BUFFER_SIZE ? tagBuffer.length : BUFFER_SIZE);
        ...
        imageSearch:
        while (raBuffer.length * numRepeats - numRepeats * 4 + num < raFileLength - 5 || !isImage) {
            if (num == raBuffer.length - 5) {
                ...
                System.out.println("Rescanning raFile starting at location " + (raBuffer.length * numRepeats - numRepeats * 4) + "\n");
                seek(raBuffer.length * numRepeats - numRepeats * 4);
                if (raBuffer.length * (numRepeats + 1) - (numRepeats + 1) * 4 > raFileLength - 5) {
                    raBuffer = new byte[(int) (raFileLength - getFilePointer())];
                }
        ...
        System.out.println("Image tag located near byte " + tagSize + " in " + (System.currentTimeMillis() - time) + "\n");
        if (!isImage) {
            System.out.println("No image tag was found for this DICOM image\n");
            return -1;
        }
        return tagSize;
    }
     ...
     * @param data Float data is broken down in bytes and stored in the byte
     * buffer.
     ...
     *
     * @param buffer Byte buffers where data is to be stored.
     * @param data Integer data is broken down in bytes and stored in the byte
     * buffer.
     * @param i Index into byte buffer.
     * @param endianess  <code>true</code> indicates big endian byte order,
     * <code>false</code> indicates little endian.
     ...
     *
     * @param buffer Byte buffers where data is to be stored.
     * @param data Long data is broken down in bytes and stored in the byte
     * buffer.
     * @param i Index into byte buffer.
     * @param endianess  <code>true</code> indicates big endian byte order,
     * <code>false</code> indicates little endian.
     ...
     *
     * @param buffer Byte buffers where data is to be stored.
     * @param data Short data is broken down in bytes and stored in the byte
     * buffer.
     * @param i Index into byte buffer.
     * @param endianess  <code>true</code> indicates big endian byte order,
     * <code>false</code> indicates little endian.
     ...
     *
     * @param buffer Byte buffers where data is to be stored.
     * @param str String containing integer data which is broken down in bytes
     * and stored in the byte buffer.
     * @param i Index into byte buffer.
     */
