...
public class FileDicomTagTable implements java.io.Serializable, Cloneable {
     ...
     */
    private DICOMFile parentFileInfo;
     ...
     */
    public FileDicomTagTable(VRtype vr_type) {
        this(null, vr_type);
    }
     ...
     */
    public FileDicomTagTable(FileDicomTagTable firstSliceTags, VRtype vr_type) {
    ...
    public static final FileDicomTag[] sortTagsList(Dictionary<FileDicomKey,FileDicomTag> tagList) {
        ...
        try {
            ...
            System.err.println("Out of Memory in FileInfoDicom.sortTagsList");
    ...
    public Object clone() {
        ...
        if (this.isReferenceTagTable) {
            newTable = new FileDicomTagTable(this.vr_type);

            if (this.childTagTables != null) {
                newTable.childTagTables = (FileDicomTagTable[]) this.childTagTables.clone();
            }
        } else {
            newTable = new FileDicomTagTable(this.referenceTagTable, this.vr_type);
        }
    ...
    public int getDataLength(boolean includeTagID) {
        ...
        while(tagsItr.hasNext()) {
        	...
        	if(nextLength != -1 && nextLength%2 != 0) {
        		System.err.println("Appending length within sequence tag");
        		nextLength++;
        	}
            datasize += nextLength;
            if(includeTagID) { //inside a sequence of elements, interested in total tag length 
                System.err.println("Tag "+nextTag.getKey()+" inside SQ has length "+nextLength);
     ...
     */
    public final void importTags(DICOMFileInputStream srcDicomInfo) {
     ...
     */
    public final void setValue(String name, Object value) throws CloneNotSupportedException {
     ...
     * @param  value   the value to set the DicomTag to
     */
    public void setValue(FileDicomKey key, Object value) throws CloneNotSupportedException {
     ...
     */
    public final void setValue(String name, Object value, int length) throws CloneNotSupportedException {
     ...
     * @param  length  the length of the tag
     */
    public final void setValue(FileDicomKey key, Object value, int length) throws CloneNotSupportedException {
     ...
     * @param  length  the length of the tag
     */
    public final void setValue(FileDicomKey key, FileDicomTag tag, Object value, int length) throws CloneNotSupportedException {
        ...
        if (tag != null) {   
            ...
            System.err.println("Tag "+key+": has already been set, overwriting");
        }

        FileDicomTagInfo info = new DicomDictionary().getInfo(key);
        ...
        if(info != null) {
            ...
            if(tag != null) {
                ...
                System.err.println("Cannot populate "+key+" because no VR has been set.\n");
                return;
            }
        }
     ...
     */
    public final void put(FileDicomTag tag) {
