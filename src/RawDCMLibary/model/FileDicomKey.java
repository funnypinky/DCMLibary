...
/**
 * FileDicomKey goes along with FileDicomTag - it is the key into the hashtable
 * that corresponds to a tag. For most of the tags it is simply a String that is
 * the unique combination of group name and element name (e.g., "0002,0010").
 * But there are some tags given in the element dictionary as "50xx,0010" or
 * "60xx,0050". The "x"s are wildcards meaning that there can be any number of
 * group names that start with "50" or "60". In this case we want the hashtable
 * to recognize the tag in order to get the pertinent information from elmdict
 * (such as "Overlay Origin" and what type of data this is) while still storing
 * the tags as unique. This is accomplished through this class and some code in
...
public class FileDicomKey {
    ...
    //~ Instance fields ------------------------------------------------------------------------------------------------
    /**
     * The dicom tag identifier in the format 'group,element'. 'x' is allowed in
     * the group number for wildcards in the dicom dictionary. E.g., '0002,0010'
     * or '50xx,00E1'.
     */
    private String key;
    ...
    //~ Constructors ---------------------------------------------------------------------------------------------------
    /**
     * Creates new key with the given String as the unique identifier. The
     * string is checked for proper format, that it specifies 2 hexadecimal
    ...
    /**
     * Creates a Key from the given group and element numbers provided. Example
     * call: new FileDicomKey(0x0080,0x001F);
     *
     * @param group Dicom tag group number.
     * @param element Dicom tag element number.
     *
     * @throws NumberFormatException If there is a problem converting the group
     * and element numbers to hexidecimal strings.
     */
    ...
    //~ Methods --------------------------------------------------------------------------------------------------------
    /**
     * Checks that the String s indeed refers to a String that is a valid key,
     * two comma-separated hexidecimal numbers in the format of &quot;[four
     * digit Group number],[four digit element number]&quot;. The Group number
     * must be four digits, but the final 2 digits may be the character
     * &quot;xx&quot;, which is used to represent a variable group.
     *
     * @param keyStr a String which is in the format 'group,element' with 'xx'
     * possibly replacing the last 2 digits of the group id
     *
     * @return true if the string input can be used to represent a DICOM key,
     * false if it cannot.
     *
     * @throws IllegalArgumentException If the inputted string cannot be used to
     * represent a DICOM key.
     */
    ...
    /**
     * Objects are equal if both are of type FileDicomKey, and both refer to the
     * same value; if the input type is a String and the String representation
     * of the key matches. This equals method will check that the keys are
     * exactly are equal or they are equal except for place-holding
     * &quot;x&quot; characters.
     *
     ...
     *
     * @throws NumberFormatException if the element is a partially defined value
     * and cannot be converted without some effort (eg, the element number is
     * ("51xx")). This is possible, but should not be discovered in elements,
     * only in groups.
     *
     * @see #getGroupNumber()
     */
     ...
     *
     * @throws NumberFormatException if the element is a partially defined value
     * and cannot be converted without some idea of what the number really is,
     * and therefor some effort. Group numbers may be only partially specified
     * (eg., "60xx").
     */
